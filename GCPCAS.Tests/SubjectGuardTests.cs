...
/// <summary>
/// Regression tests for the sync subject guard (issue #30). The 1.3.3 guard only ran
...
public class SubjectGuardTests
{
    ...
    [Fact]
    public void SubjectSurvivesGatewayRoundTrip_RejectsViaDefensiveParse_WhenNoOddBackslashRun()
    {
        // A subject with no backslashes passes HasOddBackslashRun, so this exercises the second net: the
        // defensive local X509Name parse. A bare unescaped comma produces a malformed second RDN that
        // BouncyCastle rejects, and the exception message must surface as the failure reason.
        bool survives = GCPCASClient.SubjectSurvivesGatewayRoundTrip("CN=a,b", out string failureReason);

        Assert.False(GCPCASClient.HasOddBackslashRun("CN=a,b")); // guard: not caught by the backslash check
