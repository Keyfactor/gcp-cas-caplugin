...
public class GCPCASClient : IGCPCASClient
{
    ...
    /// <param name="failureReason">Why the subject is unsafe: the structural reason from
    /// <see cref="SubjectSurvivesGatewayRoundTrip"/>, or an exception message when the PEM/subject cannot be
    /// parsed at all. <see langword="null"/> when the subject is safe.</param>
    ...
    /// </summary>
    /// <remarks>
    /// LOAD-BEARING ASSUMPTION: every <c>\</c> seen here is a literal backslash byte from the certificate,
    /// never an RFC 4514 escape introduced by <see cref="X509Certificate2.Subject"/>. This held in every
    /// observed case on net6.0/net8.0/net10.0 - .NET renders a literal backslash verbatim and <em>quotes</em>
    /// values that contain separators (e.g. <c>CN="a,b"</c>) rather than backslash-escaping them, so no
    /// single-backslash escape sequences appear. If a future/other runtime instead emitted a genuine escape
    /// such as <c>\+</c> or <c>\;</c>, that lone <c>\</c> would be counted as an odd run and the certificate
    /// would be skipped even though it is well-formed. That is a safe degradation - the cert is skipped and
    /// logged (via <c>[SYNC-SKIP]</c>) rather than aborting the sync - but it would drop a valid record from
    /// the sync, so revisit this method (e.g. restrict to runs adjacent to a value delimiter or end-of-value)
